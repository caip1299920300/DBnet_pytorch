...
class BaseDataSet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            # ii = np.array(data["img"], np.uint8)
            # ww = np.array(data["threshold_map"]* 255, np.uint8)
            # xx = np.array(data["threshold_mask"]* 255, np.uint8)
            # yy = np.array(data['shrink_mask']* 255, np.uint8)
            # zz = np.array(data['shrink_map']* 255, np.uint8)
            #
            # cv2.imshow("im",ii)
            # cv2.imshow("ww",ww)
            # cv2.imshow("xx",xx)
